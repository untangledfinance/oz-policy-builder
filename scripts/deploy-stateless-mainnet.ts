/**
 * PUBLIC deployment of the reviewed stateless execution artifacts.
 * Default is read-only. See docs/stateless-mainnet-deployment.md before --execute.
...
const SALT_TEXT = `prime-policy-interpreter:v5-execution:${INTERPRETER_HASH}`
...
function stroops(text: string): bigint {
  if (!/^\d+(\.\d{1,7})?$/.test(text))
    throw new Error('XLM amounts need nonnegative decimal with <=7 decimals')
...
if (cap <= 0n || instanceBudget <= 0n)
  throw new Error('Fee cap and instance budget must be positive')
const artifactDir = resolve(
  value('--artifact-dir', '/home/ubuntu/work/prime-stateless-wasm/wasm32v1-none/release')!
)
const receiptPath = resolve(value('--receipt', 'docs/stateless-mainnet-deployment.json')!)
...
if (cliAddress.status !== 0 || cliAddress.stdout.trim() !== SOURCE)
  throw new Error('Stellar CLI deployment identity does not match pinned public key')
...
const receipt: any = existsSync(receiptPath)
  ...
  : {
      ...
      deployer: SOURCE,
      interpreter,
      interpreterWasmHash: INTERPRETER_HASH,
      adapterWasmHash: ADAPTER_HASH,
      saltText: SALT_TEXT,
      saltHex: SALT.toString('hex'),
      transactions: [],
    }
if (
  receipt.networkPassphrase !== Networks.PUBLIC ||
  receipt.deployer !== SOURCE ||
  receipt.interpreter !== interpreter ||
  receipt.interpreterWasmHash !== INTERPRETER_HASH ||
  receipt.adapterWasmHash !== ADAPTER_HASH
)
  throw new Error('Receipt belongs to a different deployment')
function save() {
...
async function funds() {
  const [a, ledgers] = await Promise.all([
    json(`${HORIZON}/accounts/${SOURCE}`),
    json(`${HORIZON}/ledgers?order=desc&limit=1`),
  ])
  const b = a.balances.find((v: any) => v.asset_type === 'native')
  const reserve =
    BigInt(ledgers._embedded.records[0].base_reserve_in_stroops) *
    BigInt(2 + a.subentry_count + a.num_sponsoring - a.num_sponsored)
  return {
    balance: stroops(b.balance),
    reserve,
    available: stroops(b.balance) - reserve - stroops(b.selling_liabilities),
  }
}
...
async function instancePresent() {
  ...
  if (
    ...
    executable.wasmHash().toString('hex') !== INTERPRETER_HASH
  )
    throw new Error('Interpreter address contains different code')
  return true
}
async function simulate(op: xdr.Operation) {
  ...
  if (!rpc.Api.isSimulationSuccess(sim))
    throw new Error(`Simulation failed: ${'error' in sim ? sim.error : 'unexpected response'}`)
  if ('restorePreamble' in sim && sim.restorePreamble)
    throw new Error('Archived footprint requires separately reviewed restoration')
  const resourceFee = BigInt(sim.minResourceFee)
  if (BigInt(sim.transactionData.build().resourceFee().toString()) !== resourceFee)
    throw new Error('RPC resource fee fields differ')
  ...
  if (invoke.type === 'invokeHostFunction') {
    builder.clearOperations().addOperation(
      Operation.invokeHostFunction({
        ...
        func: invoke.func,
        auth: invoke.auth?.length ? invoke.auth : (sim.result?.auth ?? []),
      })
    )
  }
  const tx = builder.build()
  if (BigInt(tx.fee) !== resourceFee + INCLUSION_FEE)
    throw new Error('Fee construction changed; refusing double resource fees')
...
async function submit(
  label: string,
  prepared: Awaited<ReturnType<typeof simulate>>,
  itemBudget: bigint
) {
  if (!execute) throw new Error('Submission requires --execute')
  if (prepared.fee > itemBudget || spent + prepared.fee > cap)
    throw new Error(`Fee cap rejected ${label}`)
  if ((await funds()).available < cap - spent)
    throw new Error('Insufficient reserve-adjusted funds for remaining full fee cap')
  ...
  if (
    !signed.hash().equals(prepared.tx.hash()) ||
    !signed.signatures.some((s) =>
      Keypair.fromPublicKey(SOURCE).verify(signed.hash(), s.signature())
    )
  ) {
  ...
  const item: any = {
    label,
    hash: txHash,
    declaredFeeStroops: prepared.fee.toString(),
    simulationLedger: prepared.ledger,
    status: 'READY',
    sourceSequence: prepared.tx.sequence,
    maxTime: prepared.tx.timeBounds?.maxTime,
    unsignedEnvelopeXdr: prepared.tx.toXDR(),
  ...
  if (sent.status !== 'PENDING' && sent.status !== 'DUPLICATE')
    throw new Error(`RPC rejected ${label}: ${sent.status}; hash ${txHash}`)
  const result = await waitFor(txHash)
  item.status = result.status
  item.ledger = result.ledger
  if ('resultXdr' in result && result.resultXdr)
    item.chargedFeeStroops = result.resultXdr.feeCharged().toString()
  save()
  if (result.status !== 'SUCCESS') throw new Error(`Transaction failed: ${txHash}`)
  spent += prepared.fee
  console.log(JSON.stringify(item))
}
if (new Set(receipt.transactions.map((t: any) => t.hash)).size !== receipt.transactions.length)
  throw new Error('Duplicate receipt transaction hashes')
...
for (const prior of receipt.transactions) {
  ...
  if (result.status !== 'SUCCESS' && result.status !== 'FAILED')
    throw new Error(`Unresolved prior receipt ${prior.hash}; inspect before retry`)
  prior.status = result.status
  prior.ledger = result.ledger
  if ('resultXdr' in result && result.resultXdr)
    prior.chargedFeeStroops = result.resultXdr.feeCharged().toString()
  if (execute) save()
}
...
spent = receipt.transactions
  .filter((t: any) => t.status === 'SUCCESS' || t.status === 'FAILED')
...
const create = () =>
  Operation.createCustomContract({
    ...
    wasmHash: Buffer.from(INTERPRETER_HASH, 'hex'),
    salt: SALT,
    constructorArgs: [],
  })
let instanceEstimate: Awaited<ReturnType<typeof simulate>> | undefined
if (!hasInstance && !missing.some((a) => a.artifact.name === 'interpreter'))
  instanceEstimate = await simulate(create())
if (instanceEstimate && instanceEstimate.fee > instanceBudget)
  throw new Error('Simulated instance fee exceeds instance budget')
  ...
  JSON.stringify(
    {
      ...
      deployer: SOURCE,
      interpreter,
      interpreterDeployed: hasInstance,
      saltText: SALT_TEXT,
      saltHex: SALT.toString('hex'),
      missingWasmHashes: missing.map((a) => a.artifact.hash),
      uploadFeesXlm: xlm(uploadTotal),
      instanceFeeXlm: instanceEstimate ? xlm(instanceEstimate.fee) : null,
      instanceBudgetXlm: hasInstance ? '0' : xlm(instanceBudget),
      requiredBudgetXlm: xlm(budget),
      feeCapXlm: xlm(cap),
      balanceXlm: xlm(balance.balance),
      reserveXlm: xlm(balance.reserve),
      availableXlm: xlm(balance.available),
      priorDeclaredFeesXlm: xlm(spent),
      remainingFeeCapXlm: xlm(cap - spent),
      executionFundingShortfallXlm: xlm(
        cap - spent > balance.available ? cap - spent - balance.available : 0n
      ),
    },
    null,
    2
...
if (spent + budget > cap)
  throw new Error(
    'Prior fees plus projected uploads and bounded instance budget exceed total fee cap'
  )
if (!execute) process.exit(0)
if ((missing.length || !hasInstance) && balance.available < cap - spent)
  throw new Error('No writes: fund full fee cap above reserve before starting')
for (const { artifact } of missing) {
  if (await codePresent(artifact.hash)) continue
  const prepared = await simulate(Operation.uploadContractWasm({ wasm: artifact.wasm }))
  if (spent + prepared.fee + (hasInstance ? 0n : instanceBudget) > cap)
    throw new Error('Insufficient budget retained for interpreter instance')
  await submit(
    `upload ${artifact.name}`,
    prepared,
    cap - spent - (hasInstance ? 0n : instanceBudget)
  )
  if (!(await codePresent(artifact.hash)))
    throw new Error('Uploaded WASM was not readable after success')
}
if (!(await instancePresent()))
  await submit('deploy stateless-bound interpreter v5', await simulate(create()), instanceBudget)
...
if (
  !rpc.Api.isSimulationSuccess(versionSim) ||
  !versionSim.result ||
  scValToNative(versionSim.result.retval) !== 5
) {
  throw new Error('Deployed interpreter did not report grammar version 5')
}
receipt.verifiedAt = new Date().toISOString()
receipt.grammarVersion = 5
