...
const evidence=JSON.parse(readFileSync("docs/execution-adapter-testnet-evidence.json","utf8"));
...
for(const [name,artifact] of Object.entries({prime:"oz-account",executor:"execution-adapter",policy:"execution-policy",fixture:"test-venue"})) {
...
writeFileSync("docs/execution-adapter-testnet-receipts.json",JSON.stringify({network:"TESTNET",verifiedAt:new Date().toISOString(),records,deployedWasmProofs},(_key,value)=>typeof value==="bigint"?value.toString():value,2));
