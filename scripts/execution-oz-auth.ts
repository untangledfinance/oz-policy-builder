...
export function authPayload(signerAddresses: string[], contextRuleIds: number[]): xdr.ScVal {
  const signers = xdr.ScVal.scvMap(
    signerAddresses
      .map((a) => new xdr.ScMapEntry({ key: delegatedSigner(a), val: xdr.ScVal.scvBytes(Buffer.alloc(0)) }))
...
export function delegatedSignerEntry(accountId: string, digest: Uint8Array): xdr.SorobanAuthorizationEntry {
