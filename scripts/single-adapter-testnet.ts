...
const STATE=process.env.EXECUTOR_TESTNET_STATE??"/tmp/prime-scoped-execution-testnet-state.json";
const EVIDENCE="docs/single-adapter-testnet-evidence.json";
...
for(const [name,path] of Object.entries({
 ...
 "policy-interpreter":BUILD+"/policy_interpreter.wasm",
...
function configSc(cap:bigint) {
 ...
 const fixturePred=and(eq({kind:"call_contract"},litA(state.contracts.fixture)),eq({kind:"call_fn"},litS("act")),eq(arg(0),litA(p.prime)));
 const noAuthPred=and(eq({kind:"call_contract"},litA(state.contracts.fixture)),eq({kind:"call_fn"},litS("get")));
 plans.push({steps:[{predicate:fixturePred,authorizations:[]}],equalities:[]});
 // Test-only late failure plan. Not shipped by the production Blend builder.
 plans.push({steps:[plans[0]!.steps[0]!,{predicate:fixturePred,authorizations:[]}],equalities:[]});
 // No account auth at all for get: this leaves an unused slot that must be removed.
 plans.push({steps:[{predicate:noAuthPred,authorizations:[]}],equalities:[]});
...
await deploy("policy",await upload("policy-interpreter",BUILD+"/policy_interpreter.wasm"),[]);
await deploy("executor",await upload("execution-adapter",BUILD+"/execution_adapter.wasm"),[addr(state.contracts.prime),addr(state.contracts.policy)]);
...
await check("no-funding action",async()=>{
 ...
 assert.equal(after.fixture,7);assert.equal(after.custody,before.custody);assert.equal(after.allowance,before.allowance);assert.equal(after.scopeActive,false);assert.equal(after.prime,"0");assert.equal(after.executor,"0");
...
await check("withdraw directly to custody",async()=>{
 ...
 assert.equal(BigInt(after.custody)-BigInt(before.custody),2000000n);assert.equal(after.executor,"0");assert.equal(after.scopeActive,false);assert.equal(after.prime,"0");assert.equal(after.allowance,before.allowance);
 return {before,after};
});
await check("unused no-auth slot cleaned",async()=>{
 await exec([call(state.contracts.fixture,"get",[])],"no-auth fixture read batch");
 assert.equal((await view(state.contracts.policy,"execution_active",[addr(state.contracts.executor)])).b(),false);
 return {scopeActive:false};
});
await deny("pull-only incomplete batch denial",[pull(1000000n)]);

await deny("mismatched pull and supply amounts denial",[pull(1000000n),poolCall(500000n)]);
...
await check("human-authorized over-cap supply",async()=>{
 ...
 assert(BigInt(after.position)>BigInt(before.position));assert.equal(BigInt(before.allowance)-BigInt(after.allowance),25000000n);assert.equal(after.executor,"0");assert.equal(after.scopeActive,false);assert.equal(after.prime,"0");
...
// token rule, no execute in the tx) is denied #900 by the explicit scope guard. This
// is the property the explicit begin/end scope exists for; the old policy permitted
...
await check("standalone no-funding action denied after closed batches",async()=>{
 const result:any=await preparePrime(agent,state.contracts.fixture,"act",[addr(state.contracts.prime),u32(44)],lowRules(),true);
 assert.equal((await view(state.contracts.policy,"execution_active",[addr(state.contracts.executor)])).b(),false);
 return {error:result.denied,scopeActive:false};
});
await check("standalone venue withdrawal denied",async()=>{
 const r=map({address:addr(TOKEN),amount:int(100000n),request_type:u32(1)});
 const result:any=await preparePrime(agent,POOL,"submit",[addr(state.contracts.prime),addr(state.contracts.executor),addr(custody.publicKey()),vec([r])],lowRules(),"executor-auth");
 return {error:result.denied,blockedBy:"missing adapter authorization, before Prime policy evaluation"};
});
