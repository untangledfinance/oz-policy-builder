...
await send(
  ...
  Operation.createCustomContract({
    ...
    wasmHash: GW,
    salt: secondSalt,
    constructorArgs: [cfg(K.agent.publicKey(), [CUSTODY, sac])],
  }),
  'deploy second gate'
)
await send(
  ...
  Operation.createCustomContract({
    ...
    salt: gateSalt,
    constructorArgs: [
      cfg(adapter, [adapter, CUSTODY, sac, POOL, AQUA, OUT, prime, interpreter, gate2]),
    ],
  }),
  'deploy gate'
)
...
const must = (r: any, what: string) => {
  if (r.denied) throw new Error(`${what}: ${String(r.reason).slice(0, 150)}`)
  return r.got
}
must(
  await asPrime({
...
console.log(
  C.bold('adapter ') + adapter + C.dim(`   ${adapWasm.length}B adapter, ${gateWasm.length}B gate`)
)
...
const entry = (contract: string, args: xdr.ScVal[], subs: xdr.ScVal[] = []) =>
  vec([
    sym('Contract'),
    xdr.ScVal.scvMap([
      kv(
...
const pay = (v: xdr.ScVal) => ({
  calls: vec([call(sac, 'transfer', [addr(adapter), addr(CUSTODY), v])]),
  grants: vec([]),
})
const payloadOf = (a: string) =>
  Buffer.from(a.startsWith('C') ? StrKey.decodeContract(a) : StrKey.decodeEd25519PublicKey(a))
...
async function go(
  ...
  if (got !== expect) fails++
  console.log(
    `${got === expect ? C.dim('ok  ') : '\x1b[31mMISMATCH\x1b[0m'} ${label.padEnd(50)} ${got.padEnd(7)} ${why}`
  )
}
...
await go(
  'Blend withdraw to custody',
  {
    calls: vec([call(POOL, 'submit', wArgs)]),
    grants: vec([grant({ prime, interpreter } as any, POOL, 'submit', wArgs)]),
  },
  'PERMIT',
  [0, poolRule],
  true
  ...
  'Aquarius swap, proceeds home',
  {
    calls: vec([
      ...
      call(
        AQUA,
        'swap',
        [addr(adapter), u32v(0), u32v(1), u128(1000000n), u128(1n)],
        [tokenAuth(sac, adapter, AQUA, 1000000n)]
      ),
      call(OUT, 'transfer', [addr(adapter), addr(CUSTODY), i128v(100000n)]),
    ]),
    grants: vec([]),
  },
  'PERMIT',
...
{
  const tx = new TransactionBuilder(await server.getAccount(K.admin.publicKey()), {
    fee: FEE,
    networkPassphrase: PASSPHRASE,
  })
  ...
  console.log(
    C.bold('  cost  ') +
      (rpc.Api.isSimulationError(s)
        ? 'n/a'
        : `${s.transactionData.build().resources().instructions()} instructions`)
  )
}
...
await go(
  'a batch adding a signer to the Prime',
  { calls: vec([call(prime, 'add_signer', [u32v(0), delegatedSigner(CUSTODY)])]), grants: vec([]) },
  'REFUSE'
)
await go(
  'a batch driving the Prime own execute',
  {
    calls: vec([
      call(prime, 'execute', [
        addr(sac),
        sym('transfer'),
        vec([addr(prime), addr(CUSTODY), i128v(1n)]),
      ]),
    ]),
    grants: vec([]),
  },
...
await go(
  'drain to a stranger',
  {
    calls: vec([
      call(gate, 'pull', [addr(sac), addr(adapter), i128v(MOVE)]),
      call(sac, 'transfer', [addr(adapter), addr(STRANGER), i128v(MOVE)]),
    ]),
    grants: vec([]),
  },
  'REFUSE'
...
await go(
  'Blend proceeds to a stranger',
  {
    calls: vec([call(POOL, 'submit', sArgs)]),
    grants: vec([grant({ prime, interpreter } as any, POOL, 'submit', sArgs)]),
  },
  'REFUSE',
  [0, poolRule]
)
await go(
  'a stranger only in an auth entry',
  {
    calls: vec([
      call(gate, 'pull', [addr(sac), addr(adapter), i128v(MOVE)]),
      call(
        AQUA,
        'swap',
        [addr(adapter), u32v(0), u32v(1), u128(MOVE), u128(1n)],
        [tokenAuth(sac, adapter, STRANGER, MOVE)]
      ),
    ]),
    grants: vec([]),
  },
...
console.log(C.bold('\n── the gate ──'))
await go(
  'pull from a foreign gate listed on ours',
  { calls: vec([call(gate2, 'pull', [addr(sac), addr(adapter), i128v(MOVE)])]), grants: vec([]) },
  'PASSES'
)
await go(
  'our own gate still works',
  {
    calls: vec([
      call(gate, 'pull', [addr(sac), addr(adapter), i128v(1n)]),
      call(sac, 'transfer', [addr(adapter), addr(CUSTODY), i128v(1n)]),
    ]),
    grants: vec([]),
  },
  'PERMIT'
...
await go(
  'draw a token custody never approved',
  { calls: vec([call(gate, 'pull', [addr(OUT), addr(adapter), i128v(1n)])]), grants: vec([]) },
  'REFUSE',
  [0],
  false,
  true
...
await go(
  'pull with the gate itself as the destination',
  { calls: vec([call(gate, 'pull', [addr(sac), addr(gate), i128v(1n)])]), grants: vec([]) },
  'REFUSE',
  [0],
  false,
  true
...
await go(
  'an allowed contract as its 32-byte id',
  pay(xdr.ScVal.scvBytes(payloadOf(POOL))),
  'PASSES'
)
  ...
  'an auth entry naming an unlisted contract',
  {
    calls: vec([
      call(
        sac,
        'transfer',
        [addr(adapter), addr(CUSTODY), i128v(1n)],
        [entry(UNLISTED, [addr(adapter)])]
      ),
    ]),
    grants: vec([]),
  },
...
await go(
  'a stranger nested one sub-invocation down',
  {
    calls: vec([
      call(
        sac,
        'transfer',
        [addr(adapter), addr(CUSTODY), i128v(1n)],
        [entry(sac, [addr(adapter)], [entry(sac, [addr(STRANGER)])])]
      ),
    ]),
    grants: vec([]),
  },
...
const deployAuth = (salt: string) =>
  vec([
    sym('CreateContractWithCtorHostFn'),
    xdr.ScVal.scvMap([
      kv('constructor_args', vec([addr(CUSTODY)])),
      kv('executable', vec([sym('Wasm'), xdr.ScVal.scvBytes(AW)])),
      kv('salt', xdr.ScVal.scvBytes(hash(Buffer.from(salt)))),
    ]),
  ])
await go(
  'a deploy authorization in a grant',
  {
    calls: vec([call(sac, 'transfer', [addr(adapter), addr(CUSTODY), i128v(1n)])]),
    grants: vec([deployAuth('a')]),
  },
  'REFUSE'
)
await go(
  'a deploy authorization nested one down',
  {
    calls: vec([
      call(
        sac,
        'transfer',
        [addr(adapter), addr(CUSTODY), i128v(1n)],
        [entry(sac, [addr(adapter)], [deployAuth('b')])]
      ),
    ]),
    grants: vec([]),
  },
...
await go(
  ...
  {
    calls: vec(
      Array.from({ length: 20 }, () => call(sac, 'allowance', [addr(CUSTODY), addr(gate)]))
    ),
    grants: vec([]),
  },
  'PERMIT'
...
await go(
  'a legitimate value nested 30 deep',
  {
    calls: vec([
      call(sac, 'allowance', [addr(CUSTODY), addr(gate)], [entry(sac, [nest(30, addr(CUSTODY))])]),
    ]),
    grants: vec([]),
  },
  'PERMIT'
...
console.log(C.bold('\n── everything at once ──'))
const bulk = (last: xdr.ScVal) => ({
  calls: vec([
    ...Array.from({ length: 12 }, () =>
      call(
        sac,
        'allowance',
        [addr(CUSTODY), addr(gate)],
    ...
    call(
      ...
      'allowance',
      [addr(CUSTODY), addr(gate)],
      [entry(sac, [addr(adapter)], [entry(sac, [vec([vec([last])])])])]
    ),
  ]),
  grants: vec([]),
})
...
await go(
  'the same with a stranger as raw 32 bytes',
  bulk(xdr.ScVal.scvBytes(payloadOf(STRANGER))),
  'REFUSE'
)

console.log(C.bold('\n── the custody handover ──'))
await land(
  'the Prime admin tries to rebind',
  K.admin,
  invokeOp(adapter, 'rebind', [addr(gate2)]),
  'REJECTED'
)
...
await land(
  ...
  K.custody,
  invokeOp(adapter, 'rebind', [addr(sac)]),
  'REJECTED'
)
await land(
  'custody rebinds to a plain wallet',
  K.custody,
  invokeOp(adapter, 'rebind', [addr(STRANGER)]),
  'REJECTED'
)
await land('custody rebinds', K.custody, invokeOp(adapter, 'rebind', [addr(gate2)]), 'LANDS')
{
  ...
  const slot = (
    ((await server.getLedgerEntries(key)).entries[0]!.val as any)
      .contractData()
      .val()
      .instance()
      .storage() ?? []
  ...
  console.log(
    `${bound === gate2 ? C.dim('ok  ') : '\x1b[31mMISMATCH\x1b[0m'} ${'the binding on the ledger reads the successor'.padEnd(50)} ${bound === gate2 ? 'yes' : bound}`
  )
}
await go(
  'the old gate is no longer reachable',
  { calls: vec([call(gate, 'pull', [addr(sac), addr(adapter), i128v(1n)])]), grants: vec([]) },
  'REFUSE'
)
...
console.log(
  C.bold('custody ') + (await balance(CUSTODY)) + C.bold('   stranger ') + (await balance(STRANGER))
)
