...
assert(
  args.every((x) =>
    [
      ...
      '--fee-cap-xlm',
      '10',
      '--agent-source-prepare',
      '--agent-source-cleanup',
    ].includes(x)
  ),
  ...
  get(target, name) {
    ...
    return async (...args: any[]) => {
      for (let attempt = 0; attempt < 5; attempt++) {
        await new Promise((r) => setTimeout(r, Math.max(0, 1500 - (Date.now() - lastRpcAt))))
        lastRpcAt = Date.now()
        try {
...
function derive(deployer: string, salt: Buffer) {
  const p = xdr.HashIdPreimage.envelopeTypeContractId(
...
const prime = derive(OWNER, primeSalt),
  salt = hash(Buffer.from('prime.execution.adapter.v1')),
  adapter = derive(prime, salt)
const state: any = existsSync(EVIDENCE)
  ...
  : {
      ...
      owner: OWNER,
      agent: AGENT,
      contracts: { prime, adapter, interpreter: INTERPRETER },
      wasmHashes: { prime: OZ_HASH, adapter: ADAPTER_HASH, interpreter: INTERPRETER_HASH },
      token: TOKEN,
      pool: POOL,
      feeAndPrincipalCapStroops: TOTAL_CAP.toString(),
      maximumConcurrentPrincipalStroops: CAP.toString(),
      rules: {},
      bound: [],
      transactions: [],
      checks: [],
    }
assert.equal(state.network, PASS)
assert.equal(state.contracts.prime, prime)
assert.equal(state.contracts.interpreter, INTERPRETER)
const save = () =>
  writeFileSync(
    EVIDENCE,
...
const done = (label: string) =>
  state.transactions.find((t: any) => t.label === label && t.status === 'SUCCESS')
const spent = () =>
  state.transactions.reduce(
    (n: bigint, t: any) =>
      n +
      (t.status === 'SUCCESS' || t.status === 'FAILED'
        ? BigInt(t.chargedFeeStroops ?? t.maxFeeStroops)
        : 0n),
    0n
  )
function financialSummary() {
  ...
  return {
    depositedStroops: deposits.toString(),
    withdrawnStroops: withdrawals.toString(),
    roundingDifferenceStroops: (deposits - withdrawals).toString(),
    allFeePayersChargedStroops: spent().toString(),
  }
}
...
async function funds() {
  const fetchJson = async (url: string) => {
    ...
    assert(r.ok, 'Horizon read')
    return r.json()
  }
  const [a, l] = await Promise.all([
    fetchJson(HORIZON + '/accounts/' + OWNER),
    fetchJson(HORIZON + '/ledgers?order=desc&limit=1'),
  ])
  const b = a.balances.find((x: any) => x.asset_type === 'native'),
    reserve =
      BigInt(l._embedded.records[0].base_reserve_in_stroops) *
      BigInt(2 + a.subentry_count + a.num_sponsoring - a.num_sponsored)
  return {
    balance: amount(b.balance),
    reserve,
    available: amount(b.balance) - reserve - amount(b.selling_liabilities),
  }
}
async function ownerAccount() {
  try {
    return await server.getAccount(OWNER)
  } catch {
    // RPC wraps transient ledger reads as "Account not found"; verify fallback identity.
    const r = await fetch(HORIZON + '/accounts/' + OWNER)
    assert(r.ok, 'Owner sequence unavailable')
    const a = await r.json()
    assert.equal(a.account_id, OWNER)
    return new Account(OWNER, a.sequence)
  }
}
async function base(op: any) {
  return new TransactionBuilder(await ownerAccount(), { fee: '1000', networkPassphrase: PASS })
    .addOperation(op)
    .setTimeout(180)
    .build()
}
  ...
  return TransactionBuilder.cloneFrom(tx, {
    ...
    .addOperation(
      Operation.invokeHostFunction({
        func: op.func,
        auth: op.auth?.length ? op.auth : (sim.result?.auth ?? []),
      })
    )
    .build()
}
async function prepared(op: any) {
  const tx = await base(op),
    sim = await server.simulateTransaction(tx)
  assert(
    rpc.Api.isSimulationSuccess(sim),
    'Simulation: ' + ('error' in sim ? sim.error : 'no result')
  )
...
async function submit(tx: any, label: string) {
  ...
  assert(!done(label), 'Duplicate completed action')
  const fee = BigInt(tx.fee)
  assert(
    spent() + fee + CAP + 1_000_000n <= TOTAL_CAP,
    '10 XLM proof cap, preserving 0.1 XLM cleanup'
  )
  ...
  const cli = spawnSync(
    ...
    { input: tx.toXDR(), encoding: 'utf8', maxBuffer: 2 * 1024 * 1024 }
  )
  assert.equal(cli.status, 0, 'CLI signing failed')
  const signed: any = TransactionBuilder.fromXDR(cli.stdout.trim(), PASS)
  assert(signed.hash().equals(tx.hash()))
  assert(
    signed.signatures.some((s: any) =>
      Keypair.fromPublicKey(OWNER).verify(signed.hash(), s.signature())
    )
  )
  const item: any = {
    label,
  ...
  for (let i = 0; i < 80; i++) {
    ...
    if (r.status === 'SUCCESS' || r.status === 'FAILED') {
      ...
      item.ledger = r.ledger
      item.chargedFeeStroops = r.resultXdr.feeCharged().toString()
      item.resultXdr = r.resultXdr.toXDR('base64')
      item.envelopeXdr = r.envelopeXdr.toXDR('base64')
      item.events = (r.events?.contractEventsXdr ?? []).flat().map((e: any) => ({
        contract: e.contractId() ? Address.contract(e.contractId()).toString() : null,
        topics: e
          .body()
          .v0()
          .topics()
          .map((v: any) => scValToNative(v)),
        data: String(scValToNative(e.body().v0().data())),
      }))
      save()
      console.log(
        'RECEIPT',
        JSON.stringify({ label, hash: item.hash, status: r.status, fee: item.chargedFeeStroops })
      )
...
async function view(target: string, fn: string, args: xdr.ScVal[] = []) {
  ...
  assert(
    rpc.Api.isSimulationSuccess(sim) && sim.result,
    'View ' + fn + ' ' + ('error' in sim ? sim.error : 'empty')
  )
...
async function instance(id: string) {
  ...
  return {
    hash: v.executable().wasmHash().toString('hex'),
    storageEntries: v.storage()?.length ?? 0,
    ledger: r.latestLedger,
  }
}
async function codeLive(digest: string) {
  return (
    (
      await server.getLedgerEntries(
        xdr.LedgerKey.contractCode(
          new xdr.LedgerKeyContractCode({ hash: Buffer.from(digest, 'hex') })
        )
      )
    ).entries.length === 1
  )
...
function agentKey() {
  const line = readFileSync('/home/ubuntu/git/github.com/untangledfinance/octogate/.env', 'utf8')
    .split('\n')
  ...
  const k = Keypair.fromSecret(secret)
  assert.equal(k.publicKey(), AGENT)
  return k
}
...
async function authorize(
  ...
  assert(
    rpc.Api.isSimulationSuccess(recording),
    'Recording: ' + ('error' in recording ? recording.error : 'empty')
  )
  const original = recording.result?.auth?.find(
    (a) =>
      a.credentials().switch().name === 'sorobanCredentialsAddress' &&
      Address.fromScAddress(a.credentials().address().address()).toString() === prime
  )
  assert(original, 'Prime authorization missing')
  const seen = contexts(original.rootInvocation()),
    ids = seen.map((c) => {
      const n = rules[c.target] ?? rules['*']
      assert(n !== undefined, 'Missing rule ' + c.target)
      return n
    })
  ...
  if (operator === AGENT) {
    ...
    delegated = await authorizeEntry(
      new xdr.SorobanAuthorizationEntry({
        rootInvocation: delegated.rootInvocation(),
        credentials: xdr.SorobanCredentials.sorobanCredentialsAddress(
          new xdr.SorobanAddressCredentials({
            address: new Address(AGENT).toScAddress(),
  ...
  const auth = [
    ...
    accountEntry(
      original,
      expiry,
      authPayload([operator, ...(adapterSigner ? [adapter] : [])], ids)
    ),
    delegated,
  ]
  const tx = await base(operation(auth)),
    sim = await server.simulateTransaction(tx)
  if (deny) {
    assert(rpc.Api.isSimulationError(sim), 'Attack permitted')
    return { error: sim.error, contexts: seen } as any
  }
  assert(
    rpc.Api.isSimulationSuccess(sim),
    'Enforcing: ' + ('error' in sim ? sim.error : 'empty') + ' ids=' + ids
  )
  const ready = assemble(tx, sim)
...
const pull = (n: bigint, to = adapter) =>
  call(TOKEN, 'transfer_from', [addr(prime), addr(OWNER), addr(to), int(n)])
function pool(n: bigint, kind = 0, recipient = OWNER) {
  const auth = vec([
    ...
    map({
      context: map({
        ...
        args: vec([addr(adapter), addr(POOL), int(n)]),
      }),
      sub_invocations: vec([]),
    }),
  ])
  return call(
    ...
    [
      ...
      addr(adapter),
      addr(recipient),
      vec([map({ address: addr(TOKEN), amount: int(n), request_type: u32(kind) })]),
    ],
    kind === 0 ? [auth] : []
  )
}
...
const literalA = (value: string) => ({ kind: 'literal_address', value }),
  literalS = (value: string) => ({ kind: 'literal_symbol', value }),
  literalI = (n: bigint) => ({ kind: 'literal_i128', value: n.toString() })
const arg = (index: number) => ({ kind: 'call_arg', index }),
  eq = (left: any, right: any) => ({ op: 'eq', left, right }),
  and = (...children: any[]) => ({ op: 'and', children })
...
function installOp(name: string, target: string, signer: string, encoded: any) {
  const params = map({
    ...
    policy_admins: vec([delegatedSigner(OWNER)]),
  })
  return invoke(prime, 'add_context_rule', [
    vec([sym('CallContract'), addr(target)]),
    xdr.ScVal.scvString(name),
    xdr.ScVal.scvVoid(),
    vec([delegatedSigner(signer)]),
    xdr.ScVal.scvMap([new xdr.ScMapEntry({ key: addr(INTERPRETER), val: params })]),
  ])
}
async function install(name: string, target: string, signer: string, encoded: any) {
  if (state.rules[name] !== undefined) return
  const b = await authorize(OWNER, installOp(name, target, signer, encoded), { '*': 0 }),
    r = await submit(b.tx, 'install ' + name)
  state.rules[name] = fields(r.returnValue).get('id')!.u32()
  save()
}
async function bind(name: string) {
  ...
  const b = await authorize(
    OWNER,
    invoke(INTERPRETER, 'bind_executor', [
      vec([addr(prime), u32(state.rules[name])]),
      addr(adapter),
    ]),
    { '*': 0 }
  )
...
const rules = (name: string) => ({
  [adapter]: state.rules[name],
  [TOKEN]: state.rules.token,
  [POOL]: state.rules.pool,
})
const execution = (calls: xdr.ScVal[], name: string, deny = false) =>
  authorize(AGENT, invoke(adapter, 'execute', request(calls)), rules(name), true, deny)
async function snapshot() {
  const balance = async (a: string) => big(await view(TOKEN, 'balance', [addr(a)])).toString(),
    p = fields(await view(POOL, 'get_positions', [addr(prime)]))
  return {
    wallet: await balance(OWNER),
    prime: await balance(prime),
    adapter: await balance(adapter),
    primeAllowance: big(await view(TOKEN, 'allowance', [addr(OWNER), addr(prime)])).toString(),
    adapterAllowance: big(await view(TOKEN, 'allowance', [addr(OWNER), addr(adapter)])).toString(),
    supplyShares: (p.get('supply')?.map() ?? []).reduce((n, e) => n + big(e.val()), 0n).toString(),
    collateralEntries: p.get('collateral')?.map()?.length ?? 0,
    liabilityEntries: p.get('liabilities')?.map()?.length ?? 0,
  }
}
...
async function deny(name: string, calls: xdr.ScVal[], rootName = 'supply') {
  await check(name, async () => {
    ...
    assert(/Error\((Auth|Contract),/.test(result.error), 'Unexpected ' + result.error)
...
async function approval(n: bigint, label: string) {
  ...
  const count = state.transactions.filter(
    (t: any) => t.label === label || t.label.startsWith(label + ' retry ')
  ).length
...
const cli = spawnSync('stellar', ['keys', 'address', 'mainnet_deployer'], { encoding: 'utf8' })
assert.equal(cli.status, 0)
assert.equal(cli.stdout.trim(), OWNER)
const registryText = await fetch(REGISTRY).then(async (r) => {
    assert(r.ok)
    return r.text()
  }),
  registry = JSON.parse(registryText)
assert.equal(registry.ids.FixedV2, POOL)
assert.equal(registry.ids.poolFactoryV2, FACTORY)
assert.equal(registry.ids.XLM, TOKEN)
assert.equal((await instance(FACTORY))?.hash, FACTORY_HASH)
assert.equal((await instance(POOL))?.hash, POOL_HASH)
assert.equal(scValToNative(await view(FACTORY, 'is_pool', [addr(POOL)])), true)
assert.equal((await instance(INTERPRETER))?.hash, INTERPRETER_HASH)
assert.equal((await view(INTERPRETER, 'grammar_version')).u32(), 5)
for (const h of [OZ_HASH, INTERPRETER_HASH, ADAPTER_HASH])
  assert(await codeLive(h), 'Missing code ' + h)
const createPrime = Operation.createCustomContract({
  address: new Address(OWNER),
  salt: primeSalt,
  wasmHash: Buffer.from(OZ_HASH, 'hex'),
  constructorArgs: [vec([delegatedSigner(OWNER)]), map({})],
})
...
const tinySupplyFee = (
  await prepared(
    new Contract(POOL).call(
      ...
      addr(OWNER),
      addr(OWNER),
      vec([map({ address: addr(TOKEN), amount: int(AMOUNT), request_type: u32(0) })])
    )
  )
).fee
state.preflight = {
  ...
  registry: REGISTRY,
  registrySha256: hash(Buffer.from(registryText)).toString('hex'),
  factory: FACTORY,
  poolCode: POOL_HASH,
  factoryCode: FACTORY_HASH,
  ownerFunds: await funds(),
  freshPrimeMaxFeeStroops: freshPrimeFee,
  tinySupplyMaxFeeStroops: tinySupplyFee,
}
save()
console.log(
  'PREFLIGHT',
  JSON.stringify({ prime, adapter, ...state.preflight }, (_k, v) =>
...
if (args.includes('--agent-source-prepare')) {
  ...
  state.agentSourcePreparation = {
    at: new Date().toISOString(),
    amount: AMOUNT.toString(),
    snapshot: await snapshot(),
  }
  save()
  console.log('AGENT SOURCE READY', JSON.stringify(state.agentSourcePreparation))
  process.exit(0)
}
if (args.includes('--agent-source-cleanup')) {
  ...
  const txHash = process.env.PRIME_AGENT_SOURCE_TX
  assert(txHash && /^[a-f0-9]{64}$/.test(txHash), 'PRIME_AGENT_SOURCE_TX required')
  ...
  for (const entry of op.auth ?? [])
    if (entry.credentials().switch().name === 'sorobanCredentialsAddress')
      assert.notEqual(
        ...
        OWNER,
        'Owner authorization entry present'
      )
  const primeAuth = (op.auth ?? []).find(
    (entry: any) =>
      entry.credentials().switch().name === 'sorobanCredentialsAddress' &&
      Address.fromScAddress(entry.credentials().address().address()).toString() === prime
  )
  assert(primeAuth, 'Prime authorization missing')
  const payload = fields(primeAuth.credentials().address().signature())
  assert.deepEqual(
    payload
      .get('context_rule_ids')!
      .vec()!
      .map((x) => x.u32()),
    [state.rules.supply, state.rules.token, state.rules.pool]
  )
  assert.deepEqual(
    payload
      .get('signers')!
      .map()!
      .map((x) => Address.fromScVal(x.key().vec()![1]!).toString())
      .sort(),
    [AGENT, adapter].sort()
  )
  assert.deepEqual(
    op.func
      .invokeContract()
      .args()
      .map((x: xdr.ScVal) => x.toXDR('base64')),
    request([pull(AMOUNT), pool(AMOUNT)]).map((x) => x.toXDR('base64'))
  )
  const eventList = (r.events?.contractEventsXdr ?? []).flat().map((e: any) => ({
    contract: e.contractId() ? Address.contract(e.contractId()).toString() : null,
    topics: e
      .body()
      .v0()
  ...
  const transfers = eventList
    .filter((e: any) => e.contract === TOKEN && e.topics[0] === 'transfer')
    .map((e: any) => [e.topics[1], e.topics[2], e.data])
  assert.deepEqual(transfers, [
    [OWNER, adapter, AMOUNT.toString()],
    [adapter, POOL, AMOUNT.toString()],
  ])
  if (!done('standalone agent-source supply'))
    state.transactions.push({
      label: 'standalone agent-source supply',
      hash: txHash,
      status: 'SUCCESS',
      ledger: r.ledger,
      source: AGENT,
      ownerTransactionSignature: false,
      chargedFeeStroops: r.resultXdr.feeCharged().toString(),
      maxFeeStroops: tx.fee,
      envelopeXdr: r.envelopeXdr.toXDR('base64'),
      events: eventList,
      verifiedTokenTransfers: transfers,
    })
  state.agentSourceExecution = {
    hash: txHash,
    source: AGENT,
    onlyEnvelopeSignature: AGENT,
    ownerSignature: false,
    afterSupply: await snapshot(),
  }
  save()
  try {
    if (BigInt((await snapshot()).supplyShares) > 0n)
      await submit(
        (await execution([pool(ALL, 1)], 'withdraw')).tx,
        'standalone agent-source withdraw all'
      )
  } finally {
    await approval(0n, 'revoke standalone demo allowance')
  }
  state.final = await snapshot()
  for (const k of ['prime', 'adapter', 'primeAllowance', 'adapterAllowance', 'supplyShares'])
    assert.equal(state.final[k], '0')
  state.totalChargedFeesStroops = spent().toString()
  state.agentSourceExecution.final = state.final
  state.agentSourceExecution.cleanupAt = new Date().toISOString()
  save()
  console.log('AGENT SOURCE PROOF CLEAN', JSON.stringify(state.agentSourceExecution))
  process.exit(0)
}
if (state.complete) {
  const lateFailure = state.checks.find(
    (c: any) => c.name === 'late failure simulation leaves balances unchanged'
  )
  ...
  for (const k of ['prime', 'adapter', 'primeAllowance', 'adapterAllowance', 'supplyShares'])
    assert.equal((final as any)[k], '0')
  assert.equal(final.collateralEntries, 0)
  assert.equal(final.liabilityEntries, 0)
  assert.equal((await instance(adapter))?.hash, ADAPTER_HASH)
  assert.equal((await instance(adapter))?.storageEntries, 0)
  const observed = []
  for (const [name, id] of Object.entries(state.rules)) {
    const k = xdr.LedgerKey.contractData(
      new xdr.LedgerKeyContractData({
        contract: new Address(INTERPRETER).toScAddress(),
        key: vec([addr(prime), u32(id as number), u32(1)]),
        durability: xdr.ContractDataDurability.persistent(),
      })
    )
    const entry = (await server.getLedgerEntries(k)).entries[0]
    assert(entry, 'Missing ' + name + ' policy')
    const doc = fields(entry.val.contractData().val()),
      executor = doc.get('executor')
    assert(executor, 'Missing executor')
  ...
  save()
  console.log('READ-ONLY VERIFICATION PASSED', JSON.stringify(state.independentVerification))
  process.exit(0)
}
if (!execute) process.exit(0)
for (const t of state.transactions)
  assert(['SUCCESS', 'FAILED', 'ERROR'].includes(t.status), 'Unresolved transaction ' + t.hash)
if (state.complete) {
...
if (!(await instance(adapter))) {
  const op = (auth: xdr.SorobanAuthorizationEntry[]) =>
    Operation.createCustomContract({
      address: new Address(prime),
      salt,
      wasmHash: Buffer.from(ADAPTER_HASH, 'hex'),
      constructorArgs: [],
      auth,
    })
...
assert.equal((await instance(adapter))?.hash, ADAPTER_HASH)
assert.equal((await instance(adapter))?.storageEntries, 0)
const tokenPolicy = encodePredicate(
  and(
    eq({ kind: 'call_fn' }, literalS('transfer_from')),
    eq(arg(0), literalA(prime)),
    eq(arg(1), literalA(OWNER)),
    eq(arg(2), literalA(adapter)),
    ...positive(arg(3))
  ) as any
)
...
const poolPolicy = encodePredicate(
  and(
    ...
    eq(arg(0), literalA(prime)),
    eq(arg(1), literalA(adapter)),
    eq(arg(2), literalA(OWNER)),
...
const plans: [string, string, string, any][] = [
  ['token', TOKEN, adapter, tokenPolicy],
  ['pool', POOL, adapter, poolPolicy],
  ['supply', adapter, AGENT, root([pull(AMOUNT), pool(AMOUNT)])],
  ['withdraw', adapter, AGENT, root([pool(ALL, 1)])],
]
const forecasts = []
for (const [name, target, signer, policy] of plans)
  if (state.rules[name] === undefined)
    forecasts.push({
      name,
      fee: (await authorize(OWNER, installOp(name, target, signer, policy), { '*': 0 })).tx.fee,
    })
state.installForecast = {
  plans: forecasts,
  totalWithPriorAndCompletionReserve: (
    spent() +
    forecasts.reduce((n, x) => n + BigInt(x.fee), 0n) +
    15_000_000n
  ).toString(),
}
save()
...
assert(
  BigInt(state.installForecast.totalWithPriorAndCompletionReserve) <= TOTAL_CAP,
  'Install costs exceed proof budget'
)
// Every child is bound before an agent root becomes usable.
for (const [name, target, signer, policy] of plans) {
  await install(name, target, signer, policy)
  await bind(name)
}
try {
  await approval(
    (done('agent supply') ? 0n : AMOUNT) + (done('owner approved over-cap supply') ? 0n : CAP),
    'approve Prime only'
  )
  ...
  await check('direct pull cannot impersonate adapter', async () => {
    const b = await authorize(
      AGENT,
      invoke(TOKEN, 'transfer_from', [addr(prime), addr(OWNER), addr(adapter), int(AMOUNT)]),
      { [TOKEN]: state.rules.token },
      true,
      true
    )
    assert(/Error\((Auth|Contract),/.test(b.error))
    return { stage: 'enforcing RPC simulation', error: b.error }
  })
  await check('real Blend agent supply', async () => {
    const before = await snapshot()
    if (!done('agent supply'))
      await submit((await execution([pull(AMOUNT), pool(AMOUNT)], 'supply')).tx, 'agent supply')
    const after = await snapshot()
    assert(BigInt(after.supplyShares) > 0n)
  ...
  await check('agent withdraw all directly to wallet', async () => {
    if (!done('agent withdraw all'))
      await submit((await execution([pool(ALL, 1)], 'withdraw')).tx, 'agent withdraw all')
    const after = await snapshot()
  ...
  await check('late failure simulation leaves balances unchanged', async () => {
    ...
    const sim = await server.simulateTransaction(
      await base(new Contract(adapter).call('execute', ...request([pull(1n), fail])))
    )
    assert(rpc.Api.isSimulationError(sim))
    assert(/negative amount is not allowed/.test(sim.error), sim.error)
    assert(
      sim.error.includes('topics:[fn_return, transfer_from]'),
      'First wallet pull did not complete before failure'
    )
    assert.deepEqual(await snapshot(), before)
    return {
      stage: 'RPC simulation only',
      error: sim.error,
      unchanged: true,
      includedOnLedger: false,
    }
  })
  await check('owner approves exact over-cap batch', async () => {
    ...
    if (!done('owner approved over-cap supply'))
      await submit(
        (
          await authorize(OWNER, invoke(adapter, 'execute', request([pull(CAP), pool(CAP)])), {
            '*': 0,
          })
        ).tx,
        'owner approved over-cap supply'
      )
    const after = await snapshot()
    assert(BigInt(after.supplyShares) > 0n)
    assert.equal(after.prime, '0')
    ...
    return {
      before,
      after,
      authority: 'owner rule 0 with exact Soroban invocation signature',
      browserWalletVerified: false,
    }
  })
  await check('withdraw owner-approved deposit', async () => {
    if (!done('withdraw approved deposit'))
      await submit((await execution([pool(ALL, 1)], 'withdraw')).tx, 'withdraw approved deposit')
    const after = await snapshot()
    assert.equal(after.supplyShares, '0')
    return after
  })
  ...
  try {
    if (BigInt((await snapshot()).supplyShares) > 0n)
      await submit(
        (await authorize(OWNER, invoke(adapter, 'execute', request([pool(ALL, 1)])), { '*': 0 }))
          .tx,
        'emergency owner withdraw all'
      )
  } finally {
    await approval(0n, 'revoke remaining allowance')
  }
}
const final = await snapshot()
...
for (const [label, n] of [
  ['agent supply', AMOUNT],
  ['owner approved over-cap supply', CAP],
] as const) {
  const tx = done(label),
    transfers = tx.events
      .filter((e: any) => e.contract === TOKEN && e.topics[0] === 'transfer')
      .map((e: any) => [e.topics[1], e.topics[2], e.data])
  assert.deepEqual(transfers, [
    [OWNER, adapter, n.toString()],
    [adapter, POOL, n.toString()],
  ])
...
console.log(
  ...
  JSON.stringify({
    ...
    fees: state.totalChargedFeesStroops,
    final,
  })
)
